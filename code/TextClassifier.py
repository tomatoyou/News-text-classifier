...
# 朴素贝叶斯
    ...
    def classify(self):
        try:
            sql = 'select title from news where cate is null and admin = %s'
            try:
                cursor.execute(sql, self.account_now)
                db.commit()
            except:
            ...
            result = cursor.fetchall()
            data = [str(result[i][0]) for i in range(len(result))]
            with open('input/test.txt', 'w', encoding='utf-8') as file:
                for item in data:
                    file.write(str(item) + '\n')
            cates = predict.predict()
            for i in range(len(cates)):
                sql = 'update news set cate = %s where title = %s'
                value = (self.get_type_name(cates[i]), data[i])
                try:
                    cursor.execute(sql, value)
                    db.commit()
                except:
